"""UDS over DoIP adapter with retry and timeout policy."""
...
class UdsDoipAdapter(TransportAdapter):
    """DoIP transport adapter for live CANoe integration."""
    ...
    def send(self, request: UdsRequest) -> TransportResponse:
        ...
        while attempt <= self.policy.retry_count:
            ...
            try:
                ...
                if elapsed_s > self.policy.timeout_s:
                    raise TimeoutError(
                        f"DoIP request exceeded timeout ({elapsed_s:.3f}s > {self.policy.timeout_s:.3f}s)"
                    )
                return _to_transport_response(raw=raw, adapter="doip", attempt=attempt + 1)
        ...
        raise RuntimeError(f"DoIP diagnostic request failed after {attempt + 1} attempts") from last_error
